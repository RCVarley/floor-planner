...
const panOffset = defineModel<Point>('pan', { default: { x: 0, y: 0 } })
...
function onPointerMove(e: PointerEvent) {
  pointerPosition.value = getPointerPosition(e, panOffset, scale)

  if (activeToolName.value === 'pan' && pointerDown.value) {
    panOffset.value.x += e.movementX * -1
    panOffset.value.y += e.movementY * -1
  }
}
